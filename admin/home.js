...
const API = "https://zirconhome.onrender.com/api/home";
...
async function loadHome() {
  ...
  try {
    ...
    if (data.navbar?.logo) {
      const preview = $("navbarLogoPreview");
      if (preview) {
        preview.src = `https://zirconhome.onrender.com/${data.navbar.logo}`;
        preview.style.display = "block";
      }
    }
    ...
    if (data.about) {
      ...
      if (data.about.image) {
        ...
        if (img) { img.src = `https://zirconhome.onrender.com/${data.about.image}`; img.style.display = "block"; }
      }
    }
...
function renderSlides() {
  ...
  state.slides.forEach((s, i) => {
    ...
    if (fileStore.heroImages[i]) {
      ...
      imgPreview = `<img src="https://zirconhome.onrender.com/${s.image}" style="max-width:200px;border-radius:6px;margin-top:4px;">`;
    }
...
function renderParties() {
  ...
  state.parties.forEach((p, i) => {
    ...
    if (fileStore.partyLogos[i]) {
      ...
      logoPreview = `<img src="https://zirconhome.onrender.com/${p.logo}" style="max-width:120px;max-height:80px;border-radius:6px;margin-top:4px;">`;
    }
    const div = document.createElement("div");
...
if (data.heroSlides?.length) {
    ...
    data.heroSlides.forEach((s, idx) => {
        ...
        <div class="previewSlideLeft">
          ${s.image
            ? `<img class="previewSlideImg" src="https://zirconhome.onrender.com/${s.image}" alt="Slide ${idx+1}">`
  ...
  if (data.parties?.length) {
    ...
    data.parties.forEach((p, idx) => {
      partiesHtml += `<div class="previewPartyItem">
        ${p.logo ? `<img src="https://zirconhome.onrender.com/${p.logo}" alt="${escHtml(p.name)}">` : `<div class="previewNoImg small"><i class="fa-solid fa-handshake"></i></div>`}
  ...
  <div class="previewWrapper">
    ...
    <div class="previewBlock">
      ...
      <img class="previewLogoImg" src="https://zirconhome.onrender.com/${data.navbar.logo}" alt="Logo">
    ...
    <div class="previewBlock">
      ...
      ${data.about?.image ? `<img src="https://zirconhome.onrender.com/${data.about.image}" class="previewAboutImg">` : ''}
      <div class="previewActions" style="margin-top:10px;">
...
function deleteSlideFromPreview(id, idx) {
  ...
  if (id && id !== "undefined") {
    fetch(`https://zirconhome.onrender.com/api/home/hero/${id}`, { method: "DELETE" })
      .then(r => r.json())
      .then(res => {
...
function deletePartyFromPreview(id, idx) {
  ...
  if (id && id !== "undefined") {
    fetch(`https://zirconhome.onrender.com/api/home/party/${id}`, { method: "DELETE" })
      .then(r => r.json())
      .then(res => {
...
function buildFakeData() {
  return {
    navbar: { logo: $("navbarLogoPreview")?.src?.replace("https://zirconhome.onrender.com/", "") || "" },
