...
const plumbingList =
document.getElementById("plumbingList");
...
async function loadPlumbingWorks() {
  ...
  plumbingList.innerHTML = "";

  data.data.forEach(item => {

    plumbingList.innerHTML += `

    <div class="plumbingDataCard">
    ...
    <div class="mainActions">
      ...
      <button
        class="deleteBtn"
        onclick="deletePlumbing('${item._id}')"
      >
        Delete
      </button>
...
.forEach(box => {
  ...
  let url =
  "http://localhost:5000/api/plumbing/add";
  ...
  if(editId){
    ...
    url =
    `http://localhost:5000/api/plumbing/${editId}`;
...
async function deletePlumbing(id){
  ...
  await fetch(
    `http://localhost:5000/api/plumbing/${id}`,
    {
      method:"DELETE"
    }
  );
