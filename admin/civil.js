const form = document.getElementById("civilForm");

const civilList =
document.getElementById("civilList");
...
.forEach(box => {
  ...
  let url =
  "http://localhost:5000/api/civil/add";
  ...
  if(editId){
    ...
    url =
    `http://localhost:5000/api/civil/${editId}`;
...
async function deleteCivil(id){
  ...
  await fetch(
    `http://localhost:5000/api/civil/${id}`,
    {
      method:"DELETE"
    }
  );
