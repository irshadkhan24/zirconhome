...
const exteriorList =
document.getElementById("exteriorList");
...
async function loadExteriorWorks() {
  ...
  data.data.forEach(item => {
    ...
    <div class="mainActions">
      ...
      <button
        class="deleteBtn"
        onclick="deleteExterior('${item._id}')"
      >
        Delete
      </button>
...
.forEach(box => {
  ...
  let url =
  "http://localhost:5000/api/exterior/add";
  ...
  if(editId){
    ...
    url =
    `http://localhost:5000/api/exterior/${editId}`;
...
async function deleteExterior(id){
  ...
  await fetch(
    `http://localhost:5000/api/exterior/${id}`,
    {
      method:"DELETE"
    }
  );
