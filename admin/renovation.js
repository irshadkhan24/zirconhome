...
document.getElementById("renovationList");
...
async function loadRenovationWorks() {
  ...
  data.data.forEach(item => {
    ...
    <div class="mainActions">
      ...
      <button
        class="editBtn"
        onclick="editRenovation('${item._id}')"
      >
        Edit
      </button>
      ...
      <button
        class="deleteBtn"
        onclick="deleteRenovation('${item._id}')"
      >
        Delete
      </button>
...
.forEach(box => {
  ...
  let url =
  "http://localhost:5000/api/renovation/add";
  ...
  if(editId){
    ...
    url =
    `http://localhost:5000/api/renovation/${editId}`;
...
async function deleteRenovation(id){
  ...
  await fetch(
    `http://localhost:5000/api/renovation/${id}`,
    {
      method:"DELETE"
    }
  );
