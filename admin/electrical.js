...
document.getElementById("electricalList");
...
async function loadElectricalWorks() {
  ...
  electricalList.innerHTML = "";

  data.data.forEach(item => {

    electricalList.innerHTML += `

    <div class="electricalDataCard">
    ...
    <div class="mainActions">
      ...
      <button
        class="editBtn"
        onclick="editElectrical('${item._id}')"
      >
        Edit
      </button>
      ...
      <button
        class="deleteBtn"
        onclick="deleteElectrical('${item._id}')"
      >
        Delete
      </button>
...
.forEach(box => {
  ...
  let url =
  "http://localhost:5000/api/electrical/add";
  ...
  if(editId){
    ...
    url =
    `http://localhost:5000/api/electrical/${editId}`;
...
async function deleteElectrical(id){
  ...
  await fetch(
    `http://localhost:5000/api/electrical/${id}`,
    {
      method:"DELETE"
    }
  );
